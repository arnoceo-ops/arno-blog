...
const ALL_TOTAL = 82 // 41 strategie + 21 mensen + 20 uitvoering
  ...
  return (
    <main style={{
      ...
      <div style={{ marginBottom: '64px' }}>
        ...
        <p style={{ color: '#f0ede6', opacity: 0.35, fontSize: '13px' }}>
          {users.length} gebruiker{users.length !== 1 ? 's' : ''} actief
        </p>
      </div>

      {/* Invite */}
      <div style={{ borderTop: '1px solid #1e1e1e', paddingTop: '40px', marginBottom: '64px' }}>
        <p style={{ color: '#EE7700', fontSize: '11px', letterSpacing: '3px', marginBottom: '20px' }}>
        ...
        <div style={{ display: 'flex', gap: '16px', alignItems: 'center' }}>
          ...
          {inviteStatus && (
            <span style={{ color: '#EE7700', fontSize: '11px', letterSpacing: '2px', opacity: 0.7 }}>
              {inviteStatus}
            </span>
          )}
        </div>
      ...
      <div style={{ borderTop: '1px solid #1e1e1e', paddingTop: '40px' }}>
        <p style={{ color: '#EE7700', fontSize: '11px', letterSpacing: '3px', marginBottom: '32px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
            ...
            {users.map(u => (
              <div key={u.user_id} style={{
                ...
                <p style={{ fontSize: '12px', letterSpacing: '1px', opacity: 0.5, textTransform: 'uppercase' }}>
                  {u.role}
                </p>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                  <div style={{ flex: 1, height: '1px', backgroundColor: '#1a1a1a' }}>
                    <div style={{
                      height: '1px',
                      width: `${u.health_score || 0}%`,
                      backgroundColor: '#EE7700',
                    }} />
                  </div>
                  <span style={{ fontSize: '11px', color: '#EE7700', opacity: 0.6, minWidth: '32px' }}>
                    {u.health_score || 0}%
                  </span>
                </div>
              </div>
