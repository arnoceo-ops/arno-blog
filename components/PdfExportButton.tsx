...
function getQuestionWeight(qid: string): number { return QUESTION_WEIGHTS[qid] ?? 2 }
...
export default function PdfExportButton() {
  ...
  async function handleExport() {
    ...
    try {
      ...
      const userName = user.firstName || user.emailAddresses?.[0]?.emailAddress || 'Gebruiker'
      const laatsteAnalyse = scored.length > 0 ? new Date() : null

      // Dynamisch importeren — volledig client-side
      const { pdf } = await import('@react-pdf/renderer')
      const { CanvasPdfDocument } = await import('@/components/canvas/CanvasPdfDocument')

      const blob = await pdf(
        <CanvasPdfDocument
          answers={answers}
          healthScore={healthScore}
          kwaliteitsScore={kwaliteitsScore}
          userName={userName}
          laatsteAnalyse={laatsteAnalyse}
        />
      ).toBlob()
      ...
      a.href = url
      a.download = `rds-canvas-${new Date().getFullYear()}.pdf`
      a.click()
  ...
  return (
    <button onClick={handleExport} disabled={loading} style={btnStyle}>
      {loading ? 'PDF GENEREREN...' : '↓ EXPORT PDF'}
    </button>
  )
}